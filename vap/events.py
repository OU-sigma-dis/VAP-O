...
from utilities.utils import find_island_idx_len
...
def hold_shift_regions(
    ...
    if len(states) < 3:
        return {"shift": [], "hold": [], "long": [], "pred_shift": [], "pred_hold": [], "shift_ov": [], "pred_shift_ov": []}
    ...
    # OVERLAP SHIFTS
    shifts_ov, pred_shifts_ov, _ = get_hs_regions(
        triads=triads,
        filled_vad=filled_vad,
        triad_label=TRIAD_SHIFT_OVERLAP.to(vad.device),
        start_of=start_of,
        duration_of=duration_of,
        pre_cond_frames=pre_cond_frames,
        post_cond_frames=post_cond_frames,
        ...
        long_onset_condition_frames=long_onset_condition_frames,
        long_onset_region_frames=long_onset_region_frames,
        min_silence_frames=0,  # overlapには最小沈黙条件を適用しない
        min_context_frames=min_context_frames,
        max_frame=max_frame,
    )
...
class HoldShift:
    ...
    def __call__(
        ...
        for b in range(batch_size):
            ...
            shift_ov.append(tmp_sh["shift_ov"])
            pred_shift_ov.append(tmp_sh["pred_shift_ov"])
        return {
            "shift": shift,
            "hold": hold,
            "long": long,
...
class Backchannel:
    def __init__(
        ...
        assert prediction_region_time > 0, (
            f"Requires positive value for `prediction_region_time` got {prediction_region_time}"
        )

        assert negative_pad_left_time + negative_pad_right_time < max_time, (
...
class TurnTakingEvents:
    def __init__(self, conf: Optional[EventConfig] = None):
        ...
        assert conf.min_context_time < conf.max_time, (
    ...
    def __call__(
        ...
        # pred_shift_ov の負例も pred_hold からサンプリング
        n_pred_shift_ov_negs = self.get_total_ranges(ret["pred_shift_ov"])
        ret["pred_shift_ov_neg"] = self.sample_equal_amounts(
            n_pred_shift_ov_negs, ret["pred_hold"], event_type="pred_shift"
        )
...
if __name__ == "__main__":
    ...
    print("1. Creating dummy VAD data...")
    config = EventConfig(max_time=30)
    event_detector = TurnTakingEvents(config)

    BATCH_SIZE = 1
    N_SECONDS = 30
    FRAME_HZ = config.frame_hz
    N_FRAMES = N_SECONDS * FRAME_HZ

    vad = torch.zeros(BATCH_SIZE, N_FRAMES, 2, dtype=torch.float)
    ...
    print(f"   - VAD tensor shape: {vad.shape}")
    print("   - VAD data created with Shift, Hold, and Backchannel scenarios.")
    print("-" * 40)

    print("2. Running TurnTakingEvents detector...")
    events = event_detector(vad)
    print("   - Event detection complete.")
    print("-" * 40)

    print("3. Detected Events:")
    for event_type, batch_results in events.items():
        results_list = batch_results[0]
        print(f"\n## {event_type.upper()}")
        if not results_list:
            print("   - No events detected.")
        else:
            for i, (start, end, speaker) in enumerate(results_list):
                print(
                    f"   - Event {i + 1}: Start={start}, End={end}, Speaker={speaker}"
                )
    ...
    time_axis = np.arange(N_FRAMES) / FRAME_HZ  # 秒単位に変換

    plt.figure(figsize=(12, 4))
    plt.title("VAD and Turn-Taking Events")
    plt.xlabel("Time [s]")
    plt.ylabel("Speaker Activity")

    # スピーカーのVADを可視化（線の高さをずらして見やすく）
    plt.plot(time_axis, vad[0, :, 0].numpy() + 0.2, label="Speaker A", color="blue")
    plt.plot(time_axis, vad[0, :, 1].numpy() - 0.2, label="Speaker B", color="red")
    ...
    for event_type, batch_results in events.items():
        results_list = batch_results[0]
        color = colors.get(event_type.lower(), "gray")
        for start, end, speaker in results_list:
            start_t = start / FRAME_HZ
            end_t = end / FRAME_HZ
            height = 0.25 if speaker == 0 else -0.25
            plt.axvspan(
                start_t, end_t, color=color, alpha=0.3, label=event_type.capitalize()
            )

    plt.legend(loc="upper right")
    plt.tight_layout()
    plt.savefig("output/vad_events.png", dpi=300)
    plt.close()

    print("   - Saved visualization to output/vad_events.png")

    # valデータでイベントの数を確認
    from datasets.datamodule import VapDataModule

    dm = VapDataModule(
        train_path="../data/switchboard/vap_dataset_trans_level/train.csv",
        val_path="../data/switchboard/vap_dataset_trans_level/val.csv",
        test_path="../data/switchboard/vap_dataset_trans_level/test.csv",
        horizon=2,
        frame_hz=20,
        sample_rate=16000,
        batch_size=16,
    ...
    print("\n5. Checking events in validation data...")
    event_counts = {}
    total_batches = 0

    for batch in dm.test_dataloader():
        vad_val = batch["vad"]
        events_val = event_detector(vad_val)

        for event_type, batch_results in events_val.items():
            if event_type not in event_counts:
                event_counts[event_type] = 0
            for results_list in batch_results:
                event_counts[event_type] += len(results_list)

        total_batches += 1

    print(f"   - Total batches processed: {total_batches}")
    print("   - Event counts in validation data:")
